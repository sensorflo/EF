...
TEST(AstPrinterTest, MAKE_TEST_NAME2(
  ...
  spec = "AstCast";
  EXPECT_TOSTR_EQ(
    "bool(0)", AstCast(ObjTypeFunda::eBool, new AstNumber(0)), spec);
  ...
  EXPECT_TOSTR_EQ(";42", AstSeq(new AstNumber(42)),
    spec +
      " - As a special case, when there's only one operand, ommit the "
      "parentheses");
  EXPECT_TOSTR_EQ(
    ";(42 77)", AstSeq(new AstNumber(42), new AstNumber(77)), spec);
  ;
  EXPECT_TOSTR_EQ(";(42 77 88)",
    AstSeq(new AstNumber(42), new AstNumber(77), new AstNumber(88)), spec);
  ;

  spec = "AstIf";
  EXPECT_TOSTR_EQ("if(x 1)", AstIf(new AstSymbol("x"), new AstNumber(1)), spec);
  EXPECT_TOSTR_EQ("if(x 1 0)",
    AstIf(new AstSymbol("x"), new AstNumber(1), new AstNumber(0)), spec);

  spec = "AstWhile";
  EXPECT_TOSTR_EQ(
    "while(x 1)", AstLoop(new AstSymbol("x"), new AstNumber(1)), spec);

  spec = "AstReturn";
  EXPECT_TOSTR_EQ("return(42)", AstReturn(new AstNumber(42)), spec);

  spec = "AstDataDef";
  EXPECT_TOSTR_EQ(
    "data(foo int ())", AstDataDef("foo", ObjTypeFunda::eInt), spec);
  EXPECT_TOSTR_EQ("data(foo mut-int (42))",
    AstDataDef("foo",
      new AstObjTypeQuali(
  ...
  EXPECT_TOSTR_EQ(
    "42 77", AstCtList(new AstNumber(42), new AstNumber(77)), spec);

  spec = "AstFunDef";
  EXPECT_TOSTR_EQ("fun(foo () int 42)",
    AstFunDef("foo", AstFunDef::createArgs(),
  ...
  EXPECT_TOSTR_EQ("fun(foo () bool 1bool)",
    AstFunDef("foo", AstFunDef::createArgs(),
      ...
      new AstNumber(true, ObjTypeFunda::eBool)),
    spec);
  ...
  spec = "AstFunCall";
  EXPECT_TOSTR_EQ(
    "call(foo)", AstFunCall(new AstSymbol("foo"), new AstCtList()), spec);
  EXPECT_TOSTR_EQ("call(foo 42 77)",
    AstFunCall(new AstSymbol("foo"),
  ...
  spec = "AstObjTypePtr";
  EXPECT_TOSTR_EQ(
    "raw*int", AstObjTypePtr(new AstObjTypeSymbol(ObjTypeFunda::eInt)), spec);
  EXPECT_TOSTR_EQ("raw*foo", AstObjTypePtr(new AstObjTypeSymbol("foo")), spec);
